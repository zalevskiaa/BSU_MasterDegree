// Задача 6.1. Число различных подстрок
// 
// Найти количество различных подстрок строки $s$, используя $z$-функцию.
// Первая и единственная строка входного файла содержит строку $s$ ($1 \le |s| \le 10\,000$). Строка состоит только из строчных латинских букв.
...
int64_t Solve(const std::string &str) {
    SuffixTree tree;
    tree.Build(str);

    int64_t ans = 0;
    for (int i = 1; i < static_cast<int>(tree.nodes.size()); i++) {
        ans += Min(static_cast<int>(str.size()) - tree.nodes[i].str_pos, tree.nodes[i].str_len);
    }
